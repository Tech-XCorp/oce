...
static Standard_Boolean  PrepareEval (const Standard_Real            U,
  ...
  if (UDegree <= VDegree)
    ...
    if (CLANG_WORKAROUND_REFERENCE_IS_NULL(UMults))
      uindex -= UKLower + UDegree;
    else
      uindex  = BSplCLib::PoleIndex(UDegree,uindex,UPer,UMults);

    if (CLANG_WORKAROUND_REFERENCE_IS_NULL(VMults))
		    ...
		    TColStd_Array1OfReal&          CWeights)
{
  ...
  Standard_Boolean rational = !CLANG_WORKAROUND_REFERENCE_IS_NULL(Weights);
...
//  Standard_Integer f1,l1,f2,l2,i,j,k;
  ...
  if (!rational && !CLANG_WORKAROUND_REFERENCE_IS_NULL(CWeights)) {
...
//    ii,
  ...
  PLib::NoDerivativeEvalPolynomial(new_parameter[1],
		       ...
		       locpoles[0],
		       myPoint[0]) ;
  if (!CLANG_WORKAROUND_REFERENCE_IS_NULL(WeightsArray)) {
